...
# 从共享模块导入 verl 的替代实现
from internbootcamp.utils.verl_fallback import OpenAIFunctionToolSchema
...
def load_tool_from_config(tool_config: dict) -> Tuple[str, Callable[[str, dict], Any], Dict[str, Any]]:
    ...
    if not isinstance(raw_schema, dict):
        raise ImportError(f"Invalid tool schema: schema must be a dictionary. Got: {type(raw_schema)}")
    
    if "function" not in raw_schema:
        raise ImportError(f"Invalid tool schema: missing 'function' key")
    
    if not isinstance(raw_schema["function"], dict):
        raise ImportError(f"Invalid tool schema: 'function' must be a dictionary. Got: {type(raw_schema['function'])}")
    
    if "name" not in raw_schema["function"]:
        raise ImportError(f"Invalid tool schema: 'function' must contain 'name' field")
    
    # 创建 schema 实例
    try:
        validated_schema = OpenAIFunctionToolSchema(**raw_schema)
    except Exception as e:
        raise ImportError(f"Invalid tool schema: failed to create OpenAIFunctionToolSchema. Error: {e}")
